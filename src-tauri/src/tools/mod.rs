...
pub fn get_tool_definitions() -> Vec<Value> {
    vec![
        ...
        td!("skill_search",
            "在所有技能文件中搜索指定关键词。\
             \n搜索范围包括技能文件的内容和名称，返回匹配的文件列表。\
             \n\n适合场景：当用户的需求可能涉及某个技能但你无法确定是哪一个时，\
             \n先用 skill_search 搜索所有技能内容，找到正确的技能后再用 skill_read 读取详情。\
             \n\n注意：搜索是大小写不敏感的，且会跨所有技能文件的全部内容进行匹配。\
             \n结果中每个匹配的技能会显示文件名和匹配的内容片段。",
            &[
                ("query", "string", "要搜索的关键词（大小写不敏感，如 \"uu远程\"、\"下载\"、\"代理\"）"),
            ], ["query"]),

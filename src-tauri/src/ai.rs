...
fn build_api_messages(config: &AIConfig, messages: &[ChatMessage]) -> Vec<Value> {
    ...
    let permission_mode = config.permission_mode.as_deref().unwrap_or("safe");
    let permission_rules = match permission_mode {
        "trusted" => "当前安全模式：**信任模式** — 你的所有工具调用都会自动执行，无需用户确认。请谨慎操作。",
        _ => "当前安全模式：**安全模式** — 当你调用中高风险工具（删除文件、执行命令、编辑文件等）时，安全系统会自动拦截并弹出一个确认表单给用户。只有用户点击「确认运行」后，工具才会实际执行。\n\n\
             ⚠️ **重要：不要自己再用 ask_user 来确认**。系统会自动处理用户确认，你只需要直接调用工具即可。如果在 ask_user 之后又被系统拦截，用户会被弹窗两次。\n\n\
             当你调用工具后没有立即收到结果而是等待了一段时间，这是正常的——系统正在等待用户确认。耐心等待即可，不要重复调用。",
    };
    let base_system_prompt = format!(
        ...
        format!(
            "你是 CeBianDesktop 桌面应用的 AI 助手，通过内置工具与用户系统和文件交互。\n\n\
             核心规则：\n\
             1. 优先使用内置工具，不要用 run_command 自行写脚本绕过工具限制。\n\
             2. 工具连续失败 2 次说明方案不可行，换思路或告知用户。\n\
             3. 禁止在桌面/文档等可见目录创建临时脚本文件，临时脚本存到 %%TEMP%% 并立即删除。\n\
             4. 工具结果中的大型 base64 数据已被自动截断，不要传给其他工具。\n\n\
             {}",
            permission_rules
        )
    );
